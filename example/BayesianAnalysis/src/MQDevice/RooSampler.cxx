...
void RooSampler::Run()
{
  ...
  boost::thread resetEventCounter(boost::bind(&RooSampler::ResetEventCounter, this));
...
void RooSampler::ResetEventCounter()
{
  while ( true ) {
    try {
...
void RooSampler::Log(int intervalInMs)
{
  timestamp_t t0;
  timestamp_t t1;
...
void RooSampler::SetProperty(const int key, const string& value, const int slot/*= 0*/)
{
  switch (key) {
  default:
...
string RooSampler::GetProperty(const int key, const string& default_/*= ""*/, const int slot/*= 0*/)
{
  switch (key) {
  default:
...
void RooSampler::SetProperty(const int key, const int value, const int slot/*= 0*/)
{
  switch (key) {
  case EventSize:
    fEventSize = value;
...
int RooSampler::GetProperty(const int key, const int default_/*= 0*/, const int slot/*= 0*/)
{
  switch (key) {
  case EventSize:
    return fEventSize;
